...
class AMR_model(keras.Model):
    def __init__(self, factors=200, factors_d=20,
        ...
        self.F = tf.Variable(tf.zeros(shape=[self._batch_size * 2, self.num_image_feature]), dtype=tf.float32,
                             trainable=True)
        self.E = tf.Variable(
        ...
        self._Delta_F = tf.Variable(tf.zeros(shape=[self._batch_size * 2, self.num_image_feature]), dtype=tf.float32,
                                    trainable=False)

        self.optimizer = tf.optimizers.SGD(self._learning_rate)

    # @tf.function
    def call(self, inputs, training=None, mask=None):
        user, item, feature_ind = inputs
        ...
        feature_i = tf.squeeze(tf.nn.embedding_lookup(self.F, feature_ind)) + tf.squeeze(
            tf.nn.embedding_lookup(self._Delta_F, feature_ind))
    ...
    def train_step(self, batch, user_adv_train=False):
        ...
        self.F.assign(tf.concat([feature_pos, feature_neg], axis=0))
        self.F = tf.Variable(self.F, trainable=True)

        with tf.GradientTape() as t:
            xu_pos, gamma_u, gamma_pos, _, theta_u, beta_pos = \
                self(inputs=(user, pos, tf.range(0, self._batch_size)), training=True)
            xu_neg, _, gamma_neg, _, _, beta_neg = self(
                inputs=(user, neg, tf.range(self._batch_size, self._batch_size * 2)), training=True)
            ...
            if user_adv_train:
                ...
                # Clean Inference
                adv_xu_pos, _, _, _, _, _ = self(inputs=(user, pos, tf.range(0, self._batch_size)), training=True)
                adv_xu_neg, _, _, _, _, _ = self(inputs=(user, neg,  tf.range(self._batch_size, self._batch_size * 2)), training=True)
    ...
    def build_perturbation(self, batch):
        ...
        with tf.GradientTape() as tape_adv:
            xu_pos, gamma_u, gamma_pos, _, theta_u, beta_pos = \
                self(inputs=(user, pos,  tf.range(0, self._batch_size)), training=True)
            xu_neg, _, gamma_neg, _, _, beta_neg = self(inputs=(user, neg,  tf.range(self._batch_size, self._batch_size * 2)), training=True)
        ...
        grads = tape_adv.gradient(loss, self.trainable_variables)
        grads = tf.stop_gradient(grads[-1])

        self._Delta_F = tf.nn.l2_normalize(grads, 1) * self._eps
