...
class DVBPR_model(keras.Model):
    ...
    def predict_batch(self, start, stop, phi):
        return tf.reduce_sum(self.Tu[start:stop] * phi, axis=1)

    @tf.function
    def predict_item_batch(self, start, stop, feat):
        return tf.matmul(self.Tu[start:stop], feat, transpose_b=True)
