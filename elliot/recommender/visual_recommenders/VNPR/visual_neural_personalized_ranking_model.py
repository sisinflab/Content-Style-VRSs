...
class VNPRModel(keras.Model):
    ...
    def predict(self, inputs, training=False, **kwargs):
        ...
        """
        u, i, f = inputs
        output_1, output_2, _, _, _ = self.call(inputs=(u, i, f, i, f), training=training)
        return (output_1 + output_2) * 0.5

    @tf.function
    def predict_batch(self, start, stop, item_mf_e_1, item_mf_e_2, feature_e):
        """
        Get full predictions on the whole users/items matrix.
        ...
        mf_output_1 = tf.concat([user_mf_e * item_mf_e_1, user_v_e * feature_e], axis=1)
        mf_output_2 = tf.concat([user_mf_e * item_mf_e_2, user_v_e * feature_e], axis=1)

        mlp_output_1 = self.mlp_layers_1(mf_output_1, training=False)  # [batch_size, 1]
        mlp_output_2 = self.mlp_layers_2(mf_output_2, training=False)  # [batch_size, 1]
